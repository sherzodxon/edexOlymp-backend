...
const getDocsStatus = async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      data: { status: student.status, submission: student.docsSubmission, timeLimitSec, remainingSec, criteria, maxScore: MAX_DOCS_SCORE },
    });
  } catch (err) {
...
// Yangi guruhlar: 5-6, 7-8, 9 (9-sinf va undan yuqori)
// Eski formatlarni ham qo'llab-quvvatlaymiz: group_9_11
// ──────────────────────────────────────────────
...
function evaluateDoc(rawText, rawHtml, formatStats, criteria) {
  ...
  // Mezon yo'q — umumiy tekshiruv (max 20)
  const defaults = [
